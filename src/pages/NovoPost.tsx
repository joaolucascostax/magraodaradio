...
const tipos: { v: Tipo; label: string; desc: string; icon: any }[] = [
  { v: 'denuncia', label: 'Demanda', desc: 'Peça algo pra sua cidade', icon: AlertTriangle },
  { v: 'noticia', label: 'Notícia', desc: 'Compartilhe um fato local', icon: Newspaper },
  { v: 'projeto', label: 'Projeto', desc: 'Sugestão ou obra pra cidade', icon: Landmark },
  { v: 'discussao', label: 'Discussão', desc: 'Abra um debate com a galera', icon: MessageCircle },
];

const ETAPAS = [
  { n: 1, label: 'Categoria', icon: LayoutGrid },
  { n: 2, label: 'Texto', icon: FileText },
  { n: 3, label: 'Fotos e local', icon: Camera },
  { n: 4, label: 'Revisar', icon: Send },
];
...
export default function NovoPost() {
  ...
  const canContinue =
    etapa === 1 ? !!tipo
    : etapa === 2 ? titulo.trim().length >= 3 && corpo.trim().length >= 10
    : etapa === 3 ? !!cidadeNome?.trim()
    : true;
        ...
        <div className="rounded-[0.875rem] bg-card border border-border/60 shadow-[0_10px_40px_-15px_hsl(var(--secondary)/0.12)] overflow-hidden">
          ...
          <div className="p-5 sm:p-6 pb-4">
            <div className="flex items-center justify-between mb-4">
              <button
                type="button"
                onClick={() => (etapa > 1 ? setEtapa(etapa - 1) : navigate(-1))}
            ...
            <div className="flex items-center gap-1.5">
              {ETAPAS.map((e, i) => (
                <div key={e.n} className="flex items-center gap-1.5 flex-1 last:flex-none">
                  <div
                    className={`flex items-center justify-center w-7 h-7 rounded-full text-[11px] font-bold transition-colors shrink-0 ${
                      etapa > e.n
                  ...
                  >
                    {etapa > e.n ? <Check className="h-3.5 w-3.5" /> : e.n}
                  </div>
                  {i < ETAPAS.length - 1 && (
                    <div className="h-1 flex-1 rounded-full bg-muted overflow-hidden">
                      <div
                        className="h-full bg-primary transition-all duration-300"
                        style={{ width: etapa > e.n ? '100%' : '0%' }}
                      />
                    </div>
                  )}
                </div>
            ...
            <p className="mt-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              Etapa {etapa} de 4 · <span className="text-secondary">{ETAPAS[etapa - 1].label}</span>
            </p>
          </div>
          ...
          <div className="px-5 sm:px-6 pb-5">
              ...
              <div className="space-y-4">
                <div>
                  <h1 className="font-display text-[22px] sm:text-2xl font-extrabold tracking-[-0.02em] text-secondary leading-[1.15]">
                    O que você quer <span className="text-primary">publicar</span>?
                  </h1>
                  <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
                    Escolha a categoria. O Magrão e a equipe leem tudo.
                  </p>
                </div>
                  ...
                  {tipos.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => setTipo(t.v)}
                        className={`flex flex-col items-start gap-2 rounded-[0.875rem] border p-4 text-left transition-all active:scale-[0.98] ${
                          active
                            ? 'border-accent bg-accent/15 shadow-sm shadow-accent/20'
                            : 'border-border bg-card hover:border-accent/50'
                        }`}
                      >
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center ${active ? 'bg-accent text-accent-foreground' : 'bg-muted text-secondary'}`}>
                          <Icon className="h-4.5 w-4.5" />
                        </div>
                        <div>
                          <p className={`text-sm font-bold ${active ? 'text-secondary' : 'text-secondary/90'}`}>{t.label}</p>
            ...
            {etapa === 2 && (
              <div className="space-y-5">
                <div>
                  <h1 className="font-display text-[22px] sm:text-2xl font-extrabold tracking-[-0.02em] text-secondary leading-[1.15]">
                    Conte o que <span className="text-primary">sua cidade</span> precisa
                  </h1>
                  <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
                    Um título direto e uma descrição com detalhes.
                  </p>
                </div>
            ...
            {/* Etapa 3 — Fotos e local */}
            {etapa === 3 && (
              <div className="space-y-5">
                <div>
                  <h1 className="font-display text-[22px] sm:text-2xl font-extrabold tracking-[-0.02em] text-secondary leading-[1.15]">
                    Mostre e <span className="text-primary">localize</span>
                  </h1>
                  <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
                    Fotos ajudam a equipe a entender. O local é onde acontece.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {imagens.length < MAX_IMAGENS && (
                    <label className="group flex flex-col items-center justify-center gap-2 rounded-[0.875rem] border-2 border-dashed border-primary/25 bg-primary/[0.04] py-6 cursor-pointer hover:bg-primary/[0.08] hover:border-primary/40 transition-all">
                      ...
                      <span className="text-sm font-semibold text-secondary">
                        {imagens.length === 0 ? 'Adicionar foto' : 'Adicionar mais uma'}
                      </span>
